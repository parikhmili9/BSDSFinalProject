...
public class ClientKeyValueController implements ClientController {
  ...
  public ClientKeyValueController(RemoteServer remoteServer, Readable in, Appendable out) {
    if (remoteServer == null) {
      throw new IllegalArgumentException("The remote server is null.");
    }
    if (in == null) {
...
//      throw new BadRequestException("The key must be an integer, but got: " + inputKey);
    ...
    int key = Integer.parseInt(inputKey);
      ...
      boolean isDeleted = this.remoteServer.delete(key);
      if(!isDeleted){
...
//      throw new BadRequestException("The key must be an integer, but got: " + inputKey);
    ...
    int key = Integer.parseInt(inputKey);
    String value = this.remoteServer.get(key);
    if(value == null) {
...
//      throw new BadRequestException("The key must be an integer, but got: " + inputKey);
    ...
    try{
      int key = Integer.parseInt(inputKey);
      boolean success = this.remoteServer.put(key, value);
...
//        this.logResponse(String.format("Success: PUT(%d, %s) executed.", key, value));
        ...
        printMsg(String.format("Failed: PUT(%d, %s) not executed.", key, value));
//        this.logResponse(String.format("Failed: PUT(%d, %s) not executed.", key, value));
      }
