...
/**
 * This is the class for implementing the TCP client.
 */
public class TCPClient extends AbstractClient{
  ...
  protected void start() throws IOException {
    try {
      ...
      this.logError("Host " + this.host + " is unknown");
    } catch (SocketException se) {
      this.logError("Socket is not accessed : "+ se.getMessage() );
    } finally {
      this.logger.log("Disconnected the server.");
    }
  }
  ...
  /**
   * Driver method for TCP Client. This method also generates the log file (.txt) for TCP client.
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Logger log = ClientLogger.instance(new FileWriter("./tcp-client-log.txt"));
      new TCPClient(port, host).start();
